...
public class AdvancedEmpDeptTests
{
    ...
    public void ShouldReturnEmployeeManagerPairs()
    {
        ...
        var result = emps.Join(emps, e => e.Mgr, e => e.EmpNo, (e1, e2) => new { Employee = e1.EName, Manager = e2.EName }).ToList();
    ...
    public void ShouldJoinEmpDeptSalgrade()
    {
        ...
        // Joining dept is easy since its on 'ON E.DeptNo = D.DeptNo', however gotta use SelectMany() to pu .Where for 'BETWEEN' condition.
        var result = emps
            .Join(depts, e => e.DeptNo, d => d.DeptNo, (e, d) => new { e, d })
            .SelectMany(ob => grades.Where(s => ob.e.Sal >= s.Losal && ob.e.Sal <= s.Hisal), (ob, s) => new { ob.e, ob.d, s })
            .Select(ob => new { EName = ob.e.EName, DName = ob.d.DName, Grade = ob.s.Grade }).ToList();
