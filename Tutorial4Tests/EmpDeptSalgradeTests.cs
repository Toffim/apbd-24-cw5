...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldReturnEmployeesFromChicago()
    {
        ...
        List<Emp> result = emps.Where(e => depts.Find(d => d.DeptNo == e.DeptNo).Loc == "CHICAGO").ToList();
    ...
    public void ShouldMatchEmployeeToSalaryGrade()
    {
        ...
        // SelectMany helps with joining here
        var result = emps.SelectMany(e => grades, (e, s) => new { e, s })
            .Where(ob => ob.e.Sal >= ob.s.Losal && ob.e.Sal <= ob.s.Hisal)
            .Select(ob => new { ob.e.EName, ob.s.Grade }).ToList();
        
        // the possible alternative appraoch as well with Join().
        // e => true, s => true, because those are 'placeholders' for selecting, because
        // the selection predicates are meant to be decided later (in where)
        /*
    ...
    public void ShouldCalculateAverageSalaryPerDept()
    {
        ...
        var result = emps.GroupBy(e => e.DeptNo).Select(ob => new { DeptNo = ob.Key, AvgSal = ob.Average(e => e.Sal) });
    ...
    public void ShouldReturnEmployeesEarningMoreThanDeptAverage()
    {
        ...
        var result = emps.FindAll(e => e.Sal > emps.Where(e2 => e2.DeptNo == e.DeptNo).Average(e2 => e2.Sal)).Select(e => e.EName);
